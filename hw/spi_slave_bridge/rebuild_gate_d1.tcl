# Re-empaqueta video_vram_axi_core (gate d1) y reconstruye el slave.
...
ipx::unload_core
puts ">>> IP re-empaquetado (gate d1)."
...
# Verificar que ipshared tomo el gate d1
...
if {[llength $shared] > 0} {
    ...
    if {[string match {*vram_read_active_d1 ? vram_read_data*} $txt]} {
        puts ">>> OK: ipshared usa gate d1."
    } else {
        puts ">>> ADVERTENCIA: ipshared NO tiene gate d1!"
    }
}
