...
/*
 * Slave-side SPI transaction.
 *
 * Pre-loads the TX FIFO with the player 2 input packet, then blocks
 * until the master initiates the 24-byte full-duplex SPI transfer.
 *
 * MOSI (master → slave): official game state (24 bytes)
 * MISO (slave → master): player 2 input in bytes 0..6, padding in 7..23
 *
 * Returns:
 * 1 = valid game state received from master
 * 0 = SPI transfer failed or invalid/corrupted state packet
 */
